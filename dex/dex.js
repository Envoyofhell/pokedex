...
document.addEventListener('DOMContentLoaded', () => {
    // --- Constants ---
    const TCG_API_KEY = 'a65acbfc-55e5-4d2c-9278-253872a1bc5a'; // <<< --- PUT YOUR POKEMON TCG API KEY HERE --- >>>
    ...
    const detailPlayCryButton = document.getElementById('play-cry-button'); // Cry button
    ...
    const detailEvolutionsContainer = document.getElementById('evolution-chain-container'); // Evolution container
    const detailMatchupsContainer = document.getElementById('type-matchups-container'); // Matchup container
    ...
    // --- Dex Grid Display ---
    async function applyFiltersAndDisplayGrid() { /* Updated for sorting */ dexGridLoader.classList.remove('hidden'); pokedexGrid.innerHTML = ''; try { let baseList = await fetchGenerationList(currentGeneration); if (!baseList || baseList.length === 0) { showDexGridError(`No Pokémon found for Generation ${currentGeneration === 'all' ? 'All' : currentGeneration}.`); return; } let filteredList = baseList; if (currentTypeFilter !== 'all') { const typePokemonList = await fetchTypeData(currentTypeFilter); const typePokemonIds = new Set(typePokemonList.map(p => p.id)); filteredList = filteredList.filter(p => typePokemonIds.has(p.id)); } if (currentGridSort === 'name') { filteredList.sort((a, b) => a.name.localeCompare(b.name)); } else { filteredList.sort((a, b) => a.id - b.id); } if (filteredList.length === 0) { showDexGridError(`No ${capitalize(currentTypeFilter)} Pokémon found in Generation ${currentGeneration === 'all' ? 'All' : currentGeneration}.`); } else { displayDexGrid(filteredList); } } catch (error) { console.error("Error applying filters:", error); showDexGridError("An error occurred while filtering Pokémon."); } finally { dexGridLoader.classList.add('hidden'); } }
    ...
    function hideMovesPagination() { if (detailMovesPagination) detailMovesPagination.classList.add('hidden'); }
     ...
     function playPokemonCry() {
        ...
        } else { console.warn("No cry available for this Pokémon."); }
    }
    ...
    async function fetchEvolutionChain(url) {
        ...
        detailEvolutionsContainer.innerHTML = '<div class="spinner spinner-sm mx-auto my-4"></div><p class="text-center italic text-[var(--color-text-secondary)]">Loading evolution chain...</p>';
        try { const response = await fetch(url); if (!response.ok) throw new Error('Failed to fetch evolution chain'); const data = await response.json(); renderEvolutionChain(data.chain); } catch (error) { console.error("Error fetching evolution chain:", error); detailEvolutionsContainer.innerHTML = '<p class="text-center text-[var(--color-error)]">Could not load evolution chain.</p>'; }
    }

    function renderEvolutionChain(chainData) {
        detailEvolutionsContainer.innerHTML = '';
        const processStageRecursive = (stage, container, level = 0) => {
            const stageDiv = document.createElement('div'); stageDiv.className = 'evolution-stage';
            ...
            stageDiv.innerHTML = `<img src="${sprite}" alt="${formatName(stage.species.name)}" loading="lazy" onerror="this.src='https://placehold.co/48x48/cccccc/ffffff?text=?'"><div class="evolution-details"><p class="pokemon-name">${formatName(stage.species.name)} (#${String(pokemonId).padStart(3, '0')})</p>${stage.evolution_details.length > 0 ? `<p class="trigger">(${formatEvolutionTrigger(stage.evolution_details[0])})</p>` : ''}</div>`;
            ...
            if (stage.evolves_to.length > 0) {
                const arrowContainer = document.createElement('div'); arrowContainer.className = 'evolution-arrow-container'; // For potential styling of branches
                stage.evolves_to.forEach((nextStage, index) => {
                    const arrowDiv = document.createElement('div'); arrowDiv.className = 'evolution-arrow'; arrowDiv.innerHTML = '<i class="fas fa-arrow-down"></i>';
                    const nextStageContainer = document.createElement('div'); nextStageContainer.className = 'evolution-next-stage';
                    processStageRecursive(nextStage, nextStageContainer, level + 1);
                    arrowContainer.appendChild(arrowDiv); // Arrow first
                    arrowContainer.appendChild(nextStageContainer); // Then the next stage
                });
                 container.appendChild(arrowContainer);
            }
        };
        processStageRecursive(chainData, detailEvolutionsContainer);
    }


    function formatEvolutionTrigger(details) { /* ... (same as before) ... */ if (!details) return ''; let trigger = formatName(details.trigger?.name); if (details.item) trigger += ` w/ ${formatName(details.item.name)}`; if (details.held_item) trigger += ` holding ${formatName(details.held_item.name)}`; if (details.known_move) trigger += ` knowing ${formatName(details.known_move.name)}`; if (details.known_move_type) trigger += ` knowing ${formatName(details.known_move_type.name)} move`; if (details.location) trigger += ` at ${formatName(details.location.name)}`; if (details.min_affection) trigger += ` w/ ${details.min_affection} affection`; if (details.min_beauty) trigger += ` w/ ${details.min_beauty} beauty`; if (details.min_happiness) trigger += ` w/ ${details.min_happiness} happiness`; if (details.min_level) trigger += ` at Lv ${details.min_level}`; if (details.needs_overworld_rain) trigger += ` in rain`; if (details.party_species) trigger += ` w/ ${formatName(details.party_species.name)} in party`; if (details.party_type) trigger += ` w/ ${formatName(details.party_type.name)}-type in party`; if (details.relative_physical_stats !== null) { trigger += details.relative_physical_stats > 0 ? ' (Atk > Def)' : details.relative_physical_stats < 0 ? ' (Atk < Def)' : ' (Atk = Def)'; } if (details.time_of_day) trigger += ` at ${details.time_of_day}`; if (details.trade_species) trigger += ` when traded for ${formatName(details.trade_species.name)}`; if (details.turn_upside_down) trigger += ` turning console upside down`; return trigger; }
    ...
    function closeTcgLightbox() { /* Reinstated */ tcgLightbox.classList.remove('visible'); document.body.style.overflow = ''; lightboxCardImage.src = "data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7"; lightboxCardDetails.innerHTML = '<p>Loading details...</p>'; }
    ...
    detailPlayCryButton.addEventListener('click', playPokemonCry);
    ...
    lightboxCloseButton.addEventListener('click', closeTcgLightbox);
    tcgLightbox.addEventListener('click', (event) => { if (event.target === tcgLightbox) closeTcgLightbox(); });
    ...
    pokedexGrid.addEventListener('click', (event) => {
        ...
        if (card) {
            ...
            if (pokemonIdentifier) {
                ...
                fetchAndDisplayDetailData(pokemonIdentifier);
            } else { console.error("Could not find identifier on clicked card:", card); }
        }
    });

...
window.DexApp.DetailView.initialize = function () {
    ...
    if (this.state.elementsFound) {
        ...
        console.error(
            "Detail View Init Failed: Critical elements missing. Module will not be fully functional."
        );
    }
};
...
window.DexApp.DetailView.validateElements = function () {
    ...
    if (missingElements.length > 0) {
        console.error(
            `DetailView missing critical elements: ${missingElements.join(
                ", "
            )}`
        );
...
window.DexApp.DetailView.cacheElements = function () {
    ...
    const elementIds = [
        ...
        "game-info-tabs",
        "detail-prev-button",
        "detail-next-button", // Navigation buttons
    ];
    ...
    console.log(
        `DetailView caching: Found ${elementsFound} elements, missing ${elementsMissing} elements.`
    );

    // Cache query-selected elements safely
    try {
        if (els.tabs)
            els.detailMainTabButtons = els.tabs.querySelectorAll(".tab-button");
        if (els.contentSection)
            els.detailMainTabContents = els.contentSection.querySelectorAll(
                ".tab-content"
            );
        if (els.gameInfoTabs)
            els.detailSubTabButtons = els.gameInfoTabs.querySelectorAll(
                ".sub-tab-button"
            );
        const gameInfoTab = document.getElementById("game-info-tab");
        if (gameInfoTab)
            els.detailSubTabContents = gameInfoTab.querySelectorAll(
                ".sub-tab-content"
            );
    } catch (e) {
...
window.DexApp.DetailView.setupEventListeners = function () {
    if (!this.state.elementsFound) {
        console.warn(
            "Cannot set up DetailView event listeners: critical elements missing"
        );
        return;
    }
    ...
    addListener(elements.tabs, "click", (e) => {
        if (e.target.classList.contains("tab-button")) this.switchTab(e.target);
    });
    addListener(elements.gameInfoTabs, "click", (e) => {
...
window.DexApp.DetailView.handleKeydown = function (event) {
    ...
    if (
        !this.state.elementsFound ||
        !this.elements.detailViewLightbox.classList.contains("visible")
    ) {
        return;
    }
    ...
    switch (event.key) {
        case "ArrowLeft":
            if (
                !this.state.isLoadingNextPrev &&
                this.elements.detailPrevButton &&
                !this.elements.detailPrevButton.disabled
            ) {
        ...
        case "ArrowRight":
            if (
                !this.state.isLoadingNextPrev &&
                this.elements.detailNextButton &&
                !this.elements.detailNextButton.disabled
            ) {
        ...
        case "Escape":
            if (
                ...
                typeof window.DexApp.Lightbox.closeDetailLightbox === "function"
            ) {
...
window.DexApp.DetailView.handleStatSort = function (sortType) {
    if (!sortType) return;
    this.state.currentStatSort =
        this.state.currentStatSort === sortType && sortType !== "default"
            ? "default"
            : sortType;
    this.renderStats();
...
window.DexApp.DetailView.changeMovesPage = function (direction) {
    const totalPages = Math.ceil(
        (this.state.currentMoves?.length || 0) / this.state.movesPerPage
    );
...
window.DexApp.DetailView.fetchAndDisplayDetailData = async function (
    identifier,
    context = null
) {
    if (!identifier) {
    ...
    if (!this.state.elementsFound) {
        ...
        if (!this.state.elementsFound) {
            ...
            alert(
                `Error: Unable to display Pokémon details for "${identifier}". UI elements are missing.`
            );
            return;
        }
    }
    ...
    try {
        ...
        const data = await window.DexApp.API.fetchDetailedPokemonData(
            identifier
        );
        if (!data?.fullPokemonData)
            throw new Error(`Incomplete data received for ${identifier}`);
        ...
        if (window.DexApp.TCG?.fetchAndDisplayTcgData) {
            window.DexApp.TCG.fetchAndDisplayTcgData(
        ...
        this.showDetailError(
            error.message || `Could not load details for ${identifier}.`
        );
...
window.DexApp.DetailView.processAndDisplayDetailData = function (detailedData) {
    ...
    this.state.currentStats =
        detailedData.fullPokemonData.stats
    ...
    this.state.currentMoves =
        detailedData.fullPokemonData.moves
            ?.flatMap((moveInfo) =>
    ...
    if (detailedData.fullSpeciesData?.flavor_text_entries) {
        ...
        const formatVersionName =
            window.DexApp.Utils?.formatters?.formatVersionName ||
            ((name) =>
                name
                    ? String(name)
                          .split("-")
        ...
        const cleanFlavorText =
            window.DexApp.Utils?.formatters?.cleanFlavorText ||
            ((text) =>
        ...
        this.state.currentFlavorTextEntries = detailedData.fullSpeciesData.flavor_text_entries
            ...
            .filter(
                (e) =>
                    !seenVersions.has(e.version) && seenVersions.add(e.version)
            )
    ...
    this.populateVariantSelector(
        detailedData.varieties || [],
        detailedData.name
    );
    if (elements.variantSelectorContainer) {
        elements.variantSelectorContainer.classList.toggle(
            "hidden",
            !(detailedData.varieties && detailedData.varieties.length > 1)
        );
    }
    ...
    this.switchTab(
        elements.tabs?.querySelector(".tab-button.active") ||
            elements.detailMainTabButtons?.[0]
    );
    this.switchSubTab(
        elements.gameInfoTabs?.querySelector(".sub-tab-button.active") ||
            elements.detailSubTabButtons?.[0]
    );
...
window.DexApp.DetailView.updateNavigationButtons = function () {
    ...
    const {
        detailPrevButton: prevButton,
        detailNextButton: nextButton,
    } = this.elements;
    ...
    const canGoNext =
        canNavigate &&
        navigationContext.currentIndex < navigationContext.list.length - 1;
...
window.DexApp.DetailView.showPreviousPokemon = function () {
    ...
    if (
        isLoadingNextPrev ||
        !navigationContext ||
        navigationContext.currentIndex <= 0
    )
        return;
...
window.DexApp.DetailView.playPokemonCry = function (specificUrl = null) {
    const url =
        specificUrl ||
        this.state.currentPokemonData?.cries?.latest ||
...
window.DexApp.DetailView.switchTab = function (clickedTab) {
    if (
        !clickedTab ||
        !this.elements.detailMainTabButtons ||
        !this.elements.detailMainTabContents
    )
        return;
    ...
    this.elements.detailMainTabContents.forEach((c) =>
...
window.DexApp.DetailView.switchSubTab = function (clickedSubTab) {
    if (
        !clickedSubTab ||
        !this.elements.detailSubTabButtons ||
        !this.elements.detailSubTabContents
    )
        return;
    ...
    this.elements.detailSubTabContents.forEach((c) =>
        c.classList.toggle("active", c.id === targetId)
    );
...
window.DexApp.DetailView.updateActiveTabColor = function () {
    ...
    const darkColor =
        getComputedStyle(document.documentElement)
            .getPropertyValue("--dynamic-type-color")
...
window.DexApp.DetailView.updateActiveSubTabColor = function () {
    const activeSubTab = this.elements.gameInfoTabs?.querySelector(
...
window.DexApp.DetailView.populateVariantSelector = function (
    varieties,
...
window.DexApp.DetailView.updateDetailVisualSection = function (
    hasGenderSprites
) {
    ...
    if (this.elements.genderToggle) {
        this.elements.genderToggle.classList.toggle(
            "hidden",
            !hasGenderSprites
        );
    }
        ...
        types.forEach((ti) => {
            ...
            if (window.DexApp.Utils?.UI?.createTypeIcon) {
                typesDiv.appendChild(
                    window.DexApp.Utils.UI.createTypeIcon(ti.type.name)
                );
            } else {
                // Fallback if Utils not available
    ...
    if (this.elements.playCryButton) {
        this.elements.playCryButton.classList.toggle(
            "hidden",
...
window.DexApp.DetailView.updateDetailGameInfoTab = function (hasVariants) {
    ...
    if (window.DexApp.Utils?.UI?.updateElementText) {
        window.DexApp.Utils.UI.updateElementText(
            "pokemon-name-display",
            window.DexApp.Utils.formatters.formatName(data.name)
        );
        window.DexApp.Utils.UI.updateElementText(
            "pokemon-id",
        ...
        if (idDisplay)
            idDisplay.textContent = `#${String(data.id).padStart(3, "0")}`;

        const heightDisplay = document.getElementById("pokemon-height");
        if (heightDisplay)
            heightDisplay.textContent = `${(data.height / 10).toFixed(1)} m`;

        const weightDisplay = document.getElementById("pokemon-weight");
        if (weightDisplay)
            weightDisplay.textContent = `${(data.weight / 10).toFixed(1)} kg`;
    }

    // Show/hide variant info section
    if (this.elements.variantInfoSection) {
        this.elements.variantInfoSection.classList.toggle(
            "hidden",
            !hasVariants
        );
    }
...
window.DexApp.DetailView.updateDescription = function () {
    ...
    const entry =
        this.state.currentFlavorTextEntries[idx] ||
        this.state.currentFlavorTextEntries[0];
    ...
    if (
        !entry &&
        this.elements.gameVersionSelect &&
        this.state.currentFlavorTextEntries.length > 0
    ) {
...
window.DexApp.DetailView.renderAbilities = function () {
    ...
    if (abilities?.length > 0) {
        ...
        abilities.forEach((aInfo) => {
            ...
            item.innerHTML = `${abilityName}${
                aInfo.is_hidden
                    ? ' <span class="italic text-xs">(Hidden)</span>'
                    : ""
            }`;
            ...
            if (aInfo.ability.url) {
                fetch(aInfo.ability.url)
                    ...
                    .then((aData) => {
                        ...
                        const entry = aData.effect_entries?.find(
                    ...
                    .catch((e) =>
                        console.warn("Failed to fetch ability desc:", e)
                    );
            }
        });
    } else {
...
window.DexApp.DetailView.renderMoves = function () {
    ...
    this.state.currentMovesPage = Math.max(
        1,
        Math.min(this.state.currentMovesPage, totalPages)
    );
            ...
            fetch(move.url)
                ...
                .then((mData) => {
                    ...
                    if (mData.type?.name) {
                        ...
                        if (window.DexApp.Utils?.UI?.createTypeIcon) {
                            const typeBadge = window.DexApp.Utils.UI.createTypeIcon(
                                mData.type.name
                            );
                    ...
                    item.innerHTML += `
                        <span class="move-detail">P: ${
                            mData.power || "-"
                        }</span>
                        <span class="move-detail">Acc: ${
                            mData.accuracy || "-"
                        }</span>
                    `;
                })
    ...
    if (totalPages > 1 && pagination) {
        if (this.elements.prevMovePage)
            this.elements.prevMovePage.disabled =
                this.state.currentMovesPage === 1;
        if (this.elements.nextMovePage)
            this.elements.nextMovePage.disabled =
                this.state.currentMovesPage === totalPages;
        if (this.elements.movePageInfo)
            this.elements.movePageInfo.textContent = `Page ${this.state.currentMovesPage} of ${totalPages}`;
...
window.DexApp.DetailView.updatePokemonImage = function () {
    ...
    const placeholderUrl =
        "https://placehold.co/256x256/cccccc/333333?text=%3F";

    // Find the best available sprite based on preferences
    let url =
        art?.front_default || home?.front_default || sprites?.front_default;
    ...
    if (this.state.isShiny) {
        ...
        if (
            this.state.isFemale &&
            (home?.front_shiny_female || art?.front_shiny_female)
        ) {
    ...
    img.alt = `Image of ${this.state.isShiny ? "shiny " : ""}${
        this.state.isFemale ? "female " : ""
    }${this.state.currentPokemonData.name}`;
...
window.DexApp.DetailView.toggleShiny = function () {
    ...
    if (this.elements.shinyToggle) {
        this.elements.shinyToggle.classList.toggle(
            "active",
            this.state.isShiny
        );
    }
...
window.DexApp.DetailView.resetDetailUIState = function () {
    ...
    if (this.elements.gameVersionSelect) {
        this.elements.gameVersionSelect.innerHTML =
    ...
    window.DexApp.DetailView.showDetailError = function (message) {
        ...
        if (this.elements.detailErrorText) {
            this.elements.detailErrorText.textContent =
    ...
    console.log(
        "DetailView module loaded (v2.2.0) with improved element checks."
    );
};
